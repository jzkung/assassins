class LoadData < ActiveRecord::Migration
  def up
    ...
    kill1 = Kill.new(:lat => "37.427755", :lng => "-122.166958")
    kill1.game = game1
    kill1.target = xin
    kill1.assassin = stari
    kill1.save(:validate => false)
    kill2 = Kill.new(:lat => "37.426835", :lng => "-122.17049")
    kill2.game = game1
    kill2.target = stari
    kill2.assassin = simone
    kill2.save(:validate => false)
    kill3 = Kill.new(:lat => "37.422381", :lng => "-122.176005")
    kill3.game = game1
    kill3.target = simone
    kill3.assassin = princess
    kill3.save(:validate => false)
    kill4 = Kill.new(:lat => "37.429817", :lng => "-122.169272")
    kill4.game = game1
    kill4.target = princess
    kill4.assassin = dhruti
    kill4.save(:validate => false)
    kill5 = Kill.new(:lat => "37.426982", :lng => "-122.167566")
    kill5.game = game1
    kill5.target = dhruti
    kill5.assassin = sophie
    kill5.save(:validate => false)
    kill6 = Kill.new(:lat => "37.430612", :lng => "-122.171488")
    kill6.game = game1
    kill6.target = sophie
    kill6.assassin = emily
    kill6.save(:validate => false)

    message1 = Message.new(:post_date => "", :content => "Watch Out for Me!")
    message1.player = xin
    message1.game = game1
    message1.save(:validate => false)
    message2 = Message.new(:post_date => "", :content => "Don't let your guard down!")
    message2.player = dhruti
    message2.game = game1
    message2.save(:validate => false)
    message3 = Message.new(:post_date => "", :content => "I'm coming for you and your roommate!")
    message3.player = sophie
    message3.game = game1
    message3.save(:validate => false)
    message4 = Message.new(:post_date => "", :content => "Don't f*** around with this s***")
    message4.player = princess
    message4.game = game1
    message4.save(:validate => false)
    message5 = Message.new(:post_date => "", :content => "I'm DYING!!!")
    message5.player = xin
    message5.game = game1
    message5.save(:validate => false)

    message6 = Message.new(:post_date => "", :content => "Are you ready for this!?")
    message6.player = stari
    message6.game = game2
    message6.save(:validate => false)
    message7 = Message.new(:post_date => "", :content => "You're all gonna die!")
